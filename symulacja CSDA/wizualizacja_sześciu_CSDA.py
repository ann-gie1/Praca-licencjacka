...
# 2. Wybór izotopu
print(f"Dostępne izotopy: {df_all['Izotop'].unique()}")
...
fig = go.Figure()
dodane_do_legendy = set() # Śledzi, które kategorie są już w legendzie
...
for i, srednica in enumerate(unikalne_srednice):
    ...
    for kategoria, kolor in kolory.items():
        ...
        fig.add_trace(go.Scatter3d(
            ...
            name=f"{kategoria}", 
            legendgroup=kategoria, # Grupuje elementy w legendzie
        ...
        fig.add_trace(go.Scatter3d(
            ...
            showlegend=False # Ukryte z głównej listy legendy, ale reagują na kliknięcia grupy
        ))
