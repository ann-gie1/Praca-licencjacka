...
def main():
    ...
    # 1. Wczytanie danych (załóżmy, że folder jest tam, gdzie skrypt)
    ...
    print(f"Dostępne izotopy: {dostepne_izotopy}")
    wybrany_izotop = input("Wpisz nazwę izotopu: ")
    
    df_izotop = df_all[df_all['Izotop'] == wybrany_izotop].copy()

    if df_izotop.empty:
        print(f"Błąd: Brak danych dla izotopu {wybrany_izotop}.")
        return 1
    ...
    print(f"Gotowe! Plik zapisano jako: {plik_wyjsciowy}")
    return 0
